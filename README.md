...
> 🎬 **完整嗅探录屏演示**：  
> 若对嗅探操作流程有疑问，可直接观看项目内的实操演示视频：  
> 👉 [localhost探微信小程序的用户token和phpsessionid.mp4](images/localhost探微信小程序的用户token和phpsessionid.mp4)
...
> 🎬 **实时预约录屏演示**：  
> 了解网页预约的交互效果，可直接观看录屏：  
> 👉 [翔安校区健身房实时预约.mp4](images/翔安校区健身房实时预约.mp4)
...
- **高级配置模板**：有关底层毫秒级提前量 (`advance_ms`)、SMTP 邮件告警、Server酱等极客参数，请参考：  
  👉 [config/config.advanced.example.yaml](config/config.advanced.example.yaml)
- **核心执行全流程技术文档**：有关系统架构、双阶自愈认证原理、接口协议逆向分析，请查阅：  
