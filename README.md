...
- ⏰ **早 7 点全自动预约**：睡前挂机，次日早 06:55 自动自检登录态，07:00:00 按计划尝试预约次日名额。
...
Web 控制台只监听本机回环地址。远程访问请使用 SSH 端口转发；不要将其直接暴露到公网或局域网，因为同一台机器上的程序可访问该控制台。
...
4. 手机端点击确认后，系统换取 CAS 票据、Token 与 PHPSESSID 并写入配置；校验成功后，**页面自动跳转进入预约大厅，顶部状态变为绿色的【● 登录态有效】**。所需时间取决于认证服务和网络。
...
账号密码登录页默认勾选“保存账号密码”，以便登录失效后自动重登；保存后密码以明文写在仅当前系统账户可读的 `config/config.yaml` 中。若不需要此功能，请取消勾选，并清除此前已保存的密码。
...
2. **一键极速预约**：
   ...
   - 程序使用本地 OCR 识别验证码，然后向学校系统提交预约；实际耗时和成功率取决于验证码、网络及服务端状态。
   - 预约成功后，页面即时弹出确认提示框。

> 💡 **极速秒约体验**：  
> 网页端依次完成场次查询、预校验、验证码识别和订单提交。实际耗时取决于校园网络与服务端响应；订单提交结果不明时会停止重试，请先在官方预约记录中核对。
