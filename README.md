...
- 下载压缩包：`XMU_Gym_Booking_v1.0.0_Portable_Windows_x64.zip`；
...
*(注：系统亦保留了【⚡ HTTP 自动续登】与【🔑 微信嗅探】作为高级/兜底选项，日常使用仅需扫码登录即可。)*
...
#### 2. 启动定时挂机任务
- **绿色版用户**：直接双击文件夹内的 **`启动早7点定时抢票.bat`** 即可进入挂机倒计时；
- **源码用户**：在终端中输入命令：
  ```bash
...
3. 打开项目中的 `config/config.yaml`，填入下方配置：
   ...
   ```
4. 验证测试：在终端运行 `python main.py test-notify`（或双击测试脚本），手机微信即可秒收到一条测试提醒卡片！
